...
const JobDescription = () => {
  ...
  const applyJobHandler = async () => {
    try {
      ...
      const res = await axios.get(`https://job-portal-app-g8r9.onrender.com/api/v1/application/apply/${params.id}`);
      if (res.data.success) {
  ...
  useEffect(() => {
    const fetchSingleJob = async () => {
      try {
        axios.defaults.withCredentials = true;
        const res = await axios.get(`https://job-portal-app-g8r9.onrender.com/api/v1/job/${params.id}`);
        if (res.data.success) {
